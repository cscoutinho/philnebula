...
export const useNebula = (
    ...
    const findRelatedConcepts = useCallback(async () => {
        ...
        try {
            const { rawResponse, data: relatedData, systemInstruction, promptForLogging, model } = await nebulaService.findRelatedConcepts(ai, selectedNode, data.nodes);

            logActivity('FIND_RELATED', {
                conceptName: selectedNode.name,
                conceptId: selectedNode.id,
                provenance: { prompt: promptForLogging, systemInstruction, rawResponse, model }
            });
