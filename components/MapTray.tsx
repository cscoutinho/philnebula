...
const MapTray: React.FC<MapTrayProps> = ({ concepts, onRemove }) => {
                    ...
                    <ul className="p-2 overflow-y-auto flex-grow">
                        {concepts.map(node => (
                            <li
                                ...
                                className="group flex justify-between items-center p-2 mb-1.5 bg-gray-800 rounded-md cursor-grab active:cursor-grabbing hover:bg-gray-700 transition-colors"
                            >
                                <span className="text-sm text-gray-200">{node.name}</span>
                                <button onClick={() => onRemove(node.id)} className="opacity-0 group-hover:opacity-100 text-gray-500 hover:text-red-400 transition-opacity">
                                    <X className="w-4 h-4"/>
                                </button>
                            </li>
                        ))}
