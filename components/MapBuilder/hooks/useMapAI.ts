...
import type { MapNode, MapLink, Citation, FormalizationResult, LogicalConstruct, DefinitionAnalysisState, DefinitionResult, CounterExampleResult, MapBuilderProps, LogicalWorkbenchState, KeyVoice, Counterargument, AnalysisResult } from '../../../types';
...
export const useMapAI = ({ layout, setLayout, logActivity, nodeMap, clearSelections }: UseMapAIProps) => {
...
Respond with a JSON object containing 'justificationText' and 'citations'.`;
    ...
    }, [isSynthesizing, nodeMap, links, setLayout, clearSelections, logActivity]);
    ...
    }, [nodeMap, isAnalyzingGenealogy, logActivity, setLayout, nodes]);
    ...
    }, [isAnalyzingArgument, logActivity, nodeMap]);
    ...
    const handleAnalyzeDefinition = useCallback(async (link: MapLink, x: number, y: number) => {
        ...
        try {
            const { rawResponse, data: definitions, systemInstruction, prompt, model, usageMetadata } = await nebulaService.fetchDefinitions(sourceNode.name);
    ...
    }, [nodeMap, clearSelections, logActivity]);
    ...
    const handleHuntForCounterExamples = useCallback(async (definition: DefinitionResult) => {
        ...
        try {
            const { data: counterExamples } = await nebulaService.fetchCounterExamples(sourceNodeName, targetNodeName, definition);
