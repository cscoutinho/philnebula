...
import { SparkleIcon, StickyNoteIcon } from '../icons';
...
const NodeComponent: React.FC<NodeComponentProps> & { getFill: (node: MapNode, state: { linkingNodeId: string | number | null; selectedNodeId: string | number | null; regionSelectedNodeIds: Set<string | number>; }) => string; getStroke: (node: MapNode, state: { linkingNodeId: string | number | null; regionSelectedNodeIds: Set<string | number>; }) => string; } = ({
    node,
    isLinking,
    isSelected,
    isRegionSelected,
    ...
    const fill = NodeComponent.getFill(node, { linkingNodeId, selectedNodeId: isSelected ? node.id : null, regionSelectedNodeIds: isRegionSelected ? new Set([node.id]) : new Set() });
    const stroke = NodeComponent.getStroke(node, { linkingNodeId, regionSelectedNodeIds: isRegionSelected ? new Set([node.id]) : new Set() });
...
NodeComponent.getFill = (node: MapNode, state: { linkingNodeId: string | number | null; selectedNodeId: string | number | null; regionSelectedNodeIds: Set<string | number>; }) => {
...
NodeComponent.getStroke = (node: MapNode, state: { linkingNodeId: string | number | null; regionSelectedNodeIds: Set<string | number>; }) => {
    if (state.linkingNodeId === node.id) return "#facc15"; // yellow-400
